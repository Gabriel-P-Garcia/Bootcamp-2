...
* O projeto que está sendo desenvolvido ao longo do semestre é uma aplicação web de uma lista de compras interativa utilizando a linguagem Javascript.

* A intenção da aplicação é criar uma forma mais simplificada, centralizada e fácil de criar anotações especificamente para fazer compras, reduzindo desperdicio de papel. Outra dor que esta aplicação pretende sanar é a de evitar a perda ou esquecimento de sua lista de compras, já que a intenção da aplicação é que ela possa ser utilizada em dispositivos móveis.

* A aplicação permite o usuário inserir items em sua lista de compras em formato de checklist, disponibilizando a opção de deletar o item da lista caso o usuário desejar. 
